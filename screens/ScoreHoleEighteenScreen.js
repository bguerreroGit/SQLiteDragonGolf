...
class ScoreHoleEighteenScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            selectedIndex: null,
    ...
    getMembers() {
        ...
        db.listMembersByRoundIdHole(18, valueRound.id).then(data => {
    ...
    updateScoreText = (text, index) => {
        ...
        members[index].strokes_h18 = text;
        members[index].ultimate_sync = moment().format('YYYY-MM-DD HH:mm:ss');
        db.updateScoreMemberInHole(18, members[index]).then(result => {
            console.log('Resultado Score hole 18: ', result);
        });
        this.setState({
            members,
        });
    ...
    updateScore = (selectedIndex, index) => {
        let score = 0;
        const members = this.state.members;
        if (members[index].strokes_h18 !== '') {
            score = parseInt(members[index].strokes_h18);
        }
        ...
        members[index].strokes_h18 = score.toString();
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <TextInput
                                        style={styles.input}
                                        placeholder={'Score'}
                                        value={this.state.members[index].strokes_h18}
...
export default ScoreHoleEighteenScreen;