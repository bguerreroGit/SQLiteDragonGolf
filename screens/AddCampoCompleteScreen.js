...
export default class AddCampoCompleteScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            name: '',
    ...
    saveCampo() {
        ...
        db.addCourse(data).then((result) => {
           ...
           // Alert.alert('ID DE COURSE: ', result.insertId);
            ...
            this.setState({
                isLoading: false,
                courseId: result.rows.insertId
            });
            let courseId = result.insertId;
            this.props.navigation.navigate('TeesComplete', { courseId: courseId });
        }).catch((err) => {
            console.log(err);
            this.setState({
                isLoading: false,
