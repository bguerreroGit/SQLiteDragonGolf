...
class ScoreHoleSevenScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            selectedIndex: null,
    ...
    getMembers() {
        ...
        db.listMembersByRoundIdHole(7, valueRound.id).then(data => {
    ...
    updateScoreText = (text, index) => {
        ...
        members[index].strokes_h7 = text;
        members[index].ultimate_sync = moment().format('YYYY-MM-DD HH:mm:ss');
        db.updateScoreMemberInHole(7, members[index]).then(result => {
            console.log('Resultado Score hole 7: ', result);
        });
        this.setState({
            members,
        });
    ...
    updateScore = (selectedIndex, index) => {
        let score = 0;
        const members = this.state.members;
        if (members[index].strokes_h7 !== '') {
            score = parseInt(members[index].strokes_h7);
        }
        ...
        members[index].strokes_h7 = score.toString();
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <TextInput
                                        style={styles.input}
                                        placeholder={'Score'}
                                        value={this.state.members[index].strokes_h7}
...
export default ScoreHoleSevenScreen;