...
class ScoreHoleFiveScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            selectedIndex: null,
    ...
    getMembers() {
        ...
        db.listMembersByRoundIdHole(5, valueRound.id).then(data => {
    ...
    updateScoreText = (text, index) => {
        ...
        members[index].strokes_h5 = text;
        members[index].ultimate_sync = moment().format('YYYY-MM-DD HH:mm:ss');
        db.updateScoreMemberInHole(5, members[index]).then(result => {
            console.log('Resultado Score hole 5: ', result);
        });
        this.setState({
            members,
        });
    ...
    updateScore = (selectedIndex, index) => {
        let score = 0;
        const members = this.state.members;
        if (members[index].strokes_h5 !== '') {
            score = parseInt(members[index].strokes_h5);
        }
        ...
        members[index].strokes_h5 = score.toString();
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <TextInput
                                        style={styles.input}
                                        placeholder={'Score'}
                                        value={this.state.members[index].strokes_h5}
...
export default ScoreHoleFiveScreen;