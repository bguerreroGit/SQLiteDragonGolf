...
class ScoreHoleFourScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            selectedIndex: null,
    ...
    getMembers() {
        ...
        db.listMembersByRoundIdHole(4, valueRound.id).then(data => {
    ...
    updateScoreText = (text, index) => {
        ...
        members[index].strokes_h4 = text;
        members[index].ultimate_sync = moment().format('YYYY-MM-DD HH:mm:ss');
        db.updateScoreMemberInHole(4, members[index]).then(result => {
    ...
    updateScore = (selectedIndex, index) => {
        let score = 0;
        const members = this.state.members;
        if (members[index].strokes_h4 !== '') {
            score = parseInt(members[index].strokes_h4);
        }
        ...
        members[index].strokes_h4 = score.toString();
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <TextInput
                                        style={styles.input}
                                        placeholder={'Score'}
                                        value={this.state.members[index].strokes_h4}
...
export default ScoreHoleFourScreen;