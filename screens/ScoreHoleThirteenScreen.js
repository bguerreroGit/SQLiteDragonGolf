...
class ScoreHoleThirteenScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            selectedIndex: null,
    ...
    getMembers() {
        ...
        db.listMembersByRoundIdHole(13, valueRound.id).then(data => {
    ...
    updateScoreText = (text, index) => {
        ...
        members[index].strokes_h13 = text;
        members[index].ultimate_sync = moment().format('YYYY-MM-DD HH:mm:ss');
        db.updateScoreMemberInHole(13, members[index]).then(result => {
            console.log('Resultado Score hole 13: ', result);
        });
        this.setState({
            members,
        });
    ...
    updateScore = (selectedIndex, index) => {
        let score = 0;
        const members = this.state.members;
        if (members[index].strokes_h13 !== '') {
            score = parseInt(members[index].strokes_h13);
        }
        ...
        members[index].strokes_h13 = score.toString();
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <TextInput
                                        style={styles.input}
                                        placeholder={'Score'}
                                        value={this.state.members[index].strokes_h13}
...
export default ScoreHoleThirteenScreen;